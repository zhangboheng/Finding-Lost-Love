...
export default class Begin {
  constructor(game) {
    ...
    this.rockPlatformList = [{
      ...
      y: this.groundHeight - this.canvas.height * 0.25 + 30 * scaleY,
      direction: -1,
      isMoving: true,
      isUp: false,
    }];
    this.gearInfo = {
      x: (this.canvas.width - 32 * scaleY) * 2 - 32 * scaleY,
      y: this.groundHeight,
      statue: 'left',
      gearSound: false,
      width: 32 * scaleY,
    ...
    this.endDoorStatue = {
      x: this.canvas.width * 0.35,
      y: this.groundHeight - this.canvas.height * 0.25 - 34 * scaleY,
      width: 64 * scaleY,
      height: 64 * scaleY,
      velocityY: 0,
      gravity: 0.3,
    ...
    // 绘制陷阱的图片
    this.spikesX = this.canvas.width * 0.3;
    // 记录生命总数
    ...
    this.gameBackgroundRight.src = 'image/backgroundmirror.jpg';
    this.backButton = '';
  ...
  drawGameBackground() {
    ...
    if (this.gameBackgroundRight.complete) {
      this.context.drawImage(this.gameBackgroundRight, this.canvas.width + this.moveX, 0, this.canvas.width, this.canvas.height * 0.6)
    }
  }
  // 更新游戏背景
  ...
  drawCutRoad() {
    this.context.fillStyle = '#000000';
    this.context.fillRect(this.canvas.width * 0.3 + this.moveX, menuButtonInfo.bottom + this.canvas.height * 0.5, this.canvas.width * 0.3, this.canvas.height * 0.1);
  }
  // 绘制陷阱针刺
  drawTraps() {
    if (this.spikesImage.complete){
      let inter = Math.ceil(this.canvas.width * 0.3 / (this.spikesImage.width * scaleX / 2))
      for(let i = 0; i < inter; i++){
        this.context.drawImage(this.spikesImage, this.spikesX + this.spikesImage.width * scaleX / 2 * i + this.moveX, this.groundHeight + this.canvas.height * 0.1, this.spikesImage.width * scaleX / 2, this.spikesImage.height * scaleY / 2);
      }
    }
  }
  // 绘制岩石平台
  ...
  drawPhone(){
    if (this.phone.complete && !this.phoneObtain){
      this.context.drawImage(this.phone, (this.canvas.width - this.phone.width * scaleY) * 2 + this.moveX - this.phone.width * scaleY, this.groundHeight - this.phone.height * scaleY, this.phone.width * scaleY, this.phone.height * scaleY)
    }
  }
  // 绘制终点
  drawEndDoor(){
    if (this.endDoorStatue.show){
      if (!this.endDoorStatue.isBreak){
        ...
        if (this.endDoorTrueImage.complete) {
          this.context.drawImage(this.endDoorTrueImage, this.endDoorStatue.x + this.endDoorStatue.distence, this.endDoorStatue.y, this.endDoorStatue.width, this.endDoorStatue.height)
        }
      }
    }
  }
  ...
  updateEndDoor() {
    ...
    if (this.gearInfo.statue == 'right' && this.rockPlatformList[0].x + this.platform[0].width * scaleX / 2 <= this.endDoorStatue.x && this.endDoorStatue.y <= upOrDown && !this.endDoorStatue.follow) {
      ...
      if (this.endDoorStatue.y > upOrDown) {
        ...
        if (this.character.x <= 35 * scaleX) {
          this.endDoorStatue.lastCharacterX = 35 * scaleX
        } else {
    ...
    // 针刺和镜子的碰撞检测
    if (this.endDoorStatue.y + this.endDoorStatue.height > this.groundHeight + this.canvas.height * 0.1) {
  ...
  updateCharacter() {
    ...
    // 判断是否和断崖进行碰撞检测
    if (this.character.x + this.character.width / 2 >= this.canvas.width * 0.3 + this.moveX && this.character.x + this.character.width / 2  <= this.canvas.width * 0.6 + this.moveX && this.character.y >= this.groundHeight){
    ...
    if (this.character.x + this.character.width >= this.gearInfo.x + 10 * scaleX + this.moveX && this.character.x <= this.gearInfo.x + this.gearInfo.width - 10 * scaleX + this.moveX && this.character.y + this.character.height >= this.gearInfo.y && this.character.y <= this.gearInfo.y + this.gearInfo.height){
      ...
      if (!this.gearInfo.gearSound){
        soundManager.play('gear');
        this.gearInfo.gearSound = true;
        this.phoneObtain = true;
      }
    }
    // 判断是否与终点碰撞
    if (this.character.x <= this.endDoorStatue.x + this.endDoorStatue.width * 2 / 3 + this.endDoorStatue.distence && this.character.x + this.character.width >= this.endDoorStatue.x + this.endDoorStatue.width / 3 + this.endDoorStatue.distence && this.character.y <= this.endDoorStatue.y + this.endDoorStatue.height * 2 / 3 && this.character.y + this.character.height >= this.endDoorStatue.y + this.endDoorStatue.height / 3 && this.endDoorStatue.isBreak){
      clearInterval(this.clearSetInterval);
      this.gameWin = true;
