...
export default class Fith {
  constructor(game) {
    ...
    this.groundHeightChange = this.groundHeight - this.canvas.height * 0.06 - 20 * scaleY;
    // 绘制人物
    this.character = {
      width: 20 * scaleX,
      height: 35 * scaleY,
    ...
    this.rockPlatformList = [{
      ...
      x: this.canvas.width - 120 * scaleX,
    ...
    this.endDoorStatue = {
      x: 10 * scaleX,
      y: this.groundHeight - this.canvas.height * 0.18 - 44 * scaleY,
      width: 64 * scaleY,
      height: 64 * scaleY,
      show: true,
      isBreak: false,
    ...
    this.basiczombieInfo = {
      x: this.canvas.width - 150 * scaleX + 20 * scaleX,
      y: this.groundHeight - this.canvas.height * 0.30 - 5 * scaleY,
      width: 20 * scaleX,
      height: 35 * scaleY,
      show: true
    }
  ...
  // 绘制陷阱针刺
  drawTraps() {
    if (this.spikesImage.complete && this.spikesInfo.show) {
      this.context.fillStyle = '#000000';
      this.context.fillRect(0, this.groundHeight + this.spikesImage.height * scaleY + 20 * scaleY  - this.spikesInfo.speed, this.canvas.width, this.canvas.height * 2);
      let inter = Math.ceil(this.canvas.width / (this.spikesImage.width * scaleX / 2))
      for (let i = 0; i < inter; i++) {
        this.context.drawImage(this.spikesImage, this.spikesInfo.x + this.spikesImage.width * scaleX / 2 * i, this.groundHeight + this.spikesImage.height * scaleY - this.spikesInfo.speed, this.spikesImage.width * scaleX / 2, this.spikesImage.height * scaleY / 2);
      }
    }
  }
  // 更新针刺移动
  updateTraps(){
    if (this.spikesInfo.show){
      if (this.gearInfo.statue == 'left'){
        this.spikesInfo.speed += 0.1 * scaleY;
      }else{
        this.spikesInfo.speed -= 1 * scaleY;
      }
      if (this.groundHeight + this.spikesImage.height * scaleY / 2 - this.spikesInfo.speed > this.canvas.height){
        this.spikesInfo.show = false;
      }
    }else{
      this.spikesInfo.speed = 0;
    }
  }
  // 绘制岩石平台
  drawRockPlatform() {
    ...
    if (this.endDoorStatue.show){
      if (!this.endDoorStatue.isBreak){
        ...
        if (this.endDoorTrueImage.complete) {
          this.context.drawImage(this.endDoorTrueImage, this.endDoorStatue.x, this.endDoorStatue.y, this.endDoorStatue.width, this.endDoorStatue.height)
        }
      }
    }
  }
  ...
  updateEndDoor(){
    // 判断下面来的针刺是否和终点的底部相交
    if(this.groundHeight - this.canvas.height * 0.18 + 20 * scaleY > this.groundHeight + this.spikesImage.height * scaleY - this.spikesInfo.speed){
  ...
  updateCharacter() {
    ...
    // 判断是否和断崖进行碰撞检测
    if (this.character.x >= 0 && this.character.x + this.character.width <= this.canvas.width && this.character.y + this.character.height >= this.groundHeight + this.spikesImage.height * scaleY - this.spikesInfo.speed && this.spikesInfo.show) {
      this.character.isFall = true;
      this.character.isOnGround = false;
      this.character.y = this.groundHeight - this.spikesInfo.speed + this.character.height;
    ...
    // 判断是否与终点碰撞
    if (this.character.x <= this.endDoorStatue.x + this.endDoorStatue.width * 2 / 3 && this.character.x + this.character.width >= this.endDoorStatue.x + this.endDoorStatue.width / 3 && this.character.y <= this.endDoorStatue.y + this.endDoorStatue.height && this.character.y + this.character.height >= this.endDoorStatue.y && this.endDoorStatue.show && this.endDoorStatue.isBreak){
      clearInterval(this.clearSetInterval);
      this.gameWin = true;
