...
export default class Second {
  constructor(game) {
    ...
    this.rockPlatformList = [{
      ...
      x: (this.canvas.width - 64 * scaleX) / 6,
  ...
  drawCutRoad() {
    this.context.fillStyle = '#000000';
    this.context.fillRect(this.canvas.width * 0.25, menuButtonInfo.bottom + this.canvas.height * 0.5, this.canvas.width * 0.5, this.canvas.height * 0.1);
  }
  // 绘制陷阱针刺
  drawTraps() {
    if (this.spikesImage.complete){
      let inter = Math.ceil(this.canvas.width * 0.5 / (this.spikesImage.width * scaleX / 2))
  ...
  updateCharacter() {
    ...
    // 判断是否和断崖进行碰撞检测
    if (this.character.x + this.character.width / 2 >= this.canvas.width * 0.25 && this.character.x + this.character.width / 2  <= this.canvas.width * 0.75 && this.character.y >= this.groundHeight){
