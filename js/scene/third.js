...
export default class Third {
  constructor(game) {
    ...
    this.rockPlatformList = [{
      ...
      x: this.canvas.width - 150 * scaleX,
    ...
    this.endDoorStatue = {
      x: this.canvas.width - 150 * scaleX,
      y: this.groundHeight - this.canvas.height * 0.30 - 34 * scaleY,
      width: 64 * scaleY,
      height: 64 * scaleY,
      show: false
    }
  ...
  drawCutRoad() {
    this.context.fillStyle = '#000000';
    this.context.fillRect(this.canvas.width * 0.25, menuButtonInfo.bottom + this.canvas.height * 0.5, this.canvas.width * 0.25, this.canvas.height * 0.1);
    this.context.fillRect(this.canvas.width * 0.6, menuButtonInfo.bottom + this.canvas.height * 0.5, this.canvas.width * 0.25, this.canvas.height * 0.1);
  }
  // 绘制陷阱针刺
  drawTraps() {
    if (this.spikesImage.complete) {
      let inter = Math.ceil(this.canvas.width * 0.25 / (this.spikesImage.width * scaleX / 2))
      ...
      for (let i = 0; i < inter; i++) {
        this.context.drawImage(this.spikesImage, this.canvas.width * 0.6 + this.spikesImage.width * scaleX / 2 * i, this.groundHeight + this.canvas.height * 0.1, this.spikesImage.width * scaleX / 2, this.spikesImage.height * scaleY / 2);
      }
    }
  }
  // 绘制岩石平台
  ...
  updateCharacter() {
    ...
    // 判断是否和断崖进行碰撞检测
    if ((this.character.x + this.character.width / 2 >= this.canvas.width * 0.25 && this.character.x + this.character.width / 2  <= this.canvas.width * 0.50 || this.character.x + this.character.width / 2 >= this.canvas.width * 0.6 && this.character.x + this.character.width / 2  <= this.canvas.width * 0.85) && this.character.y >= this.groundHeight){
